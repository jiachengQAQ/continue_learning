    ...
    def get_dataset(
        ...
        if appendent is not None and len(appendent) != 0:
            if not is_dd:
                ...
                #return [DummyDataset(np.concatenate(data), np.concatenate(targets), trsf, self.use_path),MyDataset(appendent_data,appendent_targets,self.dsa_strategy)]
                return [DummyDataset(np.concatenate(data), np.concatenate(targets), trsf, self.use_path),
                        DummyDataset(appendent_data, appendent_targets, trsf, self.use_path)]
