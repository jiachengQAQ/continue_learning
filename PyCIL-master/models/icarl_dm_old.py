...
class iCaRL_DM(BaseLearner):
    ...
    #         correct, total = 0, 0
    #
    #         # 外层循环 - 遍历 train_loader
    #         for j, (_,train_inputs, train_targets) in enumerate(train_loader):
    #             train_inputs, train_targets = train_inputs.to(self._device), train_targets.to(self._device)
    #
    #             # 内层循环 - 遍历 syn_loader
    #             for i , (_,syn_inputs, syn_targets) in enumerate(syn_loader):
    #
    #                 syn_inputs, syn_targets = syn_inputs.to(self._device), syn_targets.to(self._device)
    #
    #                 # 数据合并
    #                 # 注意：这里将完整的 train_inputs 与 syn_loader 中的每个批次合并
    #                 # 您可能需要根据需要调整合并逻辑
    #                 combined_inputs = torch.cat([train_inputs, syn_inputs], dim=0)
    #                 combined_targets = torch.cat([train_targets, syn_targets], dim=0)
    #
    #                 # 模型训练部分
    #                 logits = self._network(combined_inputs)["logits"]
    #                 loss_clf = F.cross_entropy(logits, combined_targets)
    #                 loss_kd = _KD_loss(
    #                     logits[:, :self._known_classes],
    #                     self._old_network(combined_inputs)["logits"],
    #                     T,
    #                 )
    ...
    #
    #                 _, preds = torch.max(logits, dim=1)
    #                 correct += preds.eq(combined_targets.expand_as(preds)).cpu().sum()
    #                 total += len(combined_targets)
    #
    #
    #         #scheduler.step()
    #         train_acc = np.around(tensor2numpy(correct) * 100 / total, decimals=2)
    ...
    def _construct_exemplar_synthetic(self, data_manager, m):
        ...
        # self._data_memory = torch.cat((self._data_memory, real_data))
        # self._targets_memory = torch.cat((self._targets_memory, real_label))
